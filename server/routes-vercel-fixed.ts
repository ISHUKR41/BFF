...
import { storageFixed } from "./storage-vercel-fixed";
import { insertRegistrationSchema } from "@shared/schema";
import { requireAuth, generateToken } from "./index-vercel";
import { db } from "./database";
import { tournaments, registrations } from "@shared/schema";
import { eq, and } from "drizzle-orm";

export async function registerVercelRoutesFixed(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/login", async (req: Request, res: Response) => {
    try {
      ...
      const admin = await storageFixed.validateAdmin(username, password);
  ...
  app.get("/api/admin/validate", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const admin = await storageFixed.getAdmin(req.admin.id);
  ...
  app.get("/api/admin/check", async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const admin = await storageFixed.getAdmin(decoded.adminId);
  ...
  app.get("/api/tournaments", async (req: Request, res: Response) => {
    try {
      ...
      const tournamentList = await storageFixed.getAllTournaments();
  ...
  app.get("/api/tournaments/:gameType/:tournamentType", async (req: Request, res: Response) => {
    try {
      ...
      const tournament = await storageFixed.getTournament(
        gameType as any,
  ...
  app.post("/api/tournaments/reset", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const tournament = await storageFixed.resetTournament(
        gameType,
        tournamentType
      );

      // Log activity
      await storageFixed.createActivityLog({
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const tournament = await storageFixed.updateQRCode(
          gameType as any,
          tournamentType as any,
          qrCodeUrl
        );
  ...
  app.post("/api/registrations", async (req: Request, res: Response) => {
    try {
      ...
      // Create registration
      const registration = await storageFixed.createRegistration(data);
  ...
  app.get("/api/registrations", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      let registrations = await storageFixed.getAllRegistrations();
  ...
  app.get("/api/registrations/:id", requireAuth, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const registration = await storageFixed.getRegistration(id);
  ...
  app.patch("/api/registrations/:id", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const registration = await storageFixed.updateRegistrationStatus(
        id,
        status,
        req.admin!.username
      );
  ...
  app.get("/api/registrations/search/:query", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const registrations = await storageFixed.searchRegistrations(query.trim());
      res.json(registrations);
    } catch (error) {
  ...
  app.put("/api/registrations/:id/details", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const registration = await storageFixed.updateRegistrationDetails(
        id,
        updates,
        req.admin!.username
      );
  ...
  app.patch("/api/registrations/:id/notes", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const registration = await storageFixed.updateRegistrationNotes(
        id,
        notes,
        req.admin!.username
      );
  ...
  app.patch("/api/registrations/:id/flag", requireAuth, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const registration = await storageFixed.toggleRegistrationFlag(
        id,
        req.admin!.username
      );
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params;
        const registration = await storageFixed.togglePaymentVerification(
          id,
          req.admin!.username
        );
  ...
  app.delete("/api/registrations/:id", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const success = await storageFixed.deleteRegistration(id);
  ...
  app.post("/api/registrations/bulk/approve", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const results = await Promise.allSettled(
        ids.map((id) =>
          storageFixed.updateRegistrationStatus(id, "approved", req.admin!.username)
        )
      );
  ...
  app.post("/api/registrations/bulk/reject", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const results = await Promise.allSettled(
        ids.map((id) =>
          storageFixed.updateRegistrationStatus(id, "rejected", req.admin!.username)
        )
      );
  ...
  app.get("/api/activity-logs", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const logs = await storageFixed.getAllActivityLogs(maxLimit);
      res.json(logs);
    } catch (error) {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const logs = await storageFixed.getActivityLogsByTarget(
          targetType,
          targetId
        );
        res.json(logs);
