...
export class ImprovedStorage implements IStorage {
    ...
    try {
      ...
      for (const type of ["solo", "duo", "squad"] as const) {
        ...
        if (existing.length === 0) {
          ...
          // Update count based on actual registrations
          // Only update if we're not in a serverless environment or if the count is clearly wrong
          if (!process.env.VERCEL) {
            const actualCount = await this.getActualRegistrationCount(
              "bgmi",
              type
            );
            if (existing[0].registeredCount !== actualCount) {
              await db
                .update(tournaments)
  ...
  async getAllTournaments(): Promise<Tournament[]> {
    try {
      // Refresh counts before returning, but only in non-serverless environments
      // In serverless (Vercel), we avoid this to prevent performance issues
      if (!process.env.VERCEL) {
        await this.refreshAllTournamentCounts();
      }
  ...
  private async refreshAllTournamentCounts(): Promise<void> {
    try {
...
export const storage = new ImprovedStorage();